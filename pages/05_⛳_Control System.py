...
def run_pendulum_pid():
    ...
    st.write(
        ...
        ${\displaystyle \hspace{2em} \ddot{x} = \frac{1}{M + m \sin^2\theta}\left(F - b\dot{x} - m\ell\dot{\theta}^2\sin\theta + mg\sin\theta\cos\theta\right)}$
        
        ${\displaystyle \hspace{2em} \ddot{\theta} = \frac{1}{(M + m \sin^2\theta)\ell}\left((F - b\dot{x})\cos\theta - m\ell\dot{\theta}^2\sin\theta\cos\theta + (M+m)g\sin\theta\right)}$
        """
    )
    ...
    st.write(
        ...
        ${\displaystyle \hspace{2em} F = K_p e + K_i \int_0^t e(\tau)d\tau + K_d \dot{e}}~~~$ where $~~e = \theta_{ref} - \theta =  0 - \theta$

        ${\displaystyle \hspace{2em} \hat{F}(s) = -\frac{K_d s^2 + K_p s + K_i}{s}\,\hat{\theta}(s)}$
        """
    )
    ...
    st.write(
        ...
        ${\displaystyle \hspace{2em} ~~~\:\,\,\ddot{x} \approx -\frac{b}{M}\dot{x} + \frac{m}{M}g\theta + \frac{1}{M}F}$
        
        ${\displaystyle \hspace{2em} ~~~\:\,\,\ddot{\theta} \approx -\frac{b}{M\ell}\dot{x} + \frac{M+m}{M\ell}g\theta + \frac{1}{M\ell}F}$

        ${\displaystyle \hspace{2em} \hat{\theta}(s) \approx \frac{\frac{1}{M\ell}s}{s^3 + \frac{b}{M}s^2 - \frac{(M+m)g}{M\ell}s - \frac{bg}{M\ell}}\hat{F}(s) = -\frac{\frac{1}{m\ell}(K_ds^2 + K_ps + K_i)}{s^3 + \frac{b}{M}s^2 - \frac{(M+m)g}{M\ell}s - \frac{bg}{M\ell}}\hat{\theta}(s)}$
    
        ${\displaystyle \hspace{2em} \left(s^3 + (\frac{b}{M} + \frac{b}{M\ell})s^2 + (\frac{K_p}{M\ell} - \frac{(M+m)g}{M\ell})s + (\frac{K_i}{M\ell} - \frac{bg}{M\ell})\right)\hat{\theta}(s) \approx 0}$
        """
    )
    st.write("")
    st.write(
        ...
        ${\displaystyle \hspace{2em} K_p =  a_{m2}M\ell + (M+m)g}$,
        ${\displaystyle ~\,K_i = a_{m3}M\ell + bg}$,
        ${\displaystyle ~\,K_d = a_{m1}M\ell - b\ell}$
